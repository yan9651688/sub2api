...
当前版本：`0.2.2-llmrelay.2`。维护分支：`codex/llmrepay`。
...
使用 `<上游版本>-llmrelay.<修订号>`，固定保留 `llmrelay` 拼写。本次为 `0.2.2-llmrelay.2`；同一上游版本后续依次使用 `.3`、`.4`。升级上游基础版本时，将前面的版本同步为实际基础版本，并从修订号 `.1` 开始。

程序版本的源码入口为 `backend/cmd/server/VERSION`。发布时同步该文件、Git 标签、构建参数和镜像标签。Git 标签采用完整版本字符串，例如 `0.2.2-llmrelay.2`。历史的 `availability.1` 和临时的 `memory.1` 名称仅保留在历史发布记录中。

## 基础与功能

基于上游 `v0.2.2`，提交 `5485f368b29d05adb95a00f71801c7c23d8f48af`。
